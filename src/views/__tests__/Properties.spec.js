...
describe('Properties.vue', () => {
  ...
  it('renders no PropertyCards when API returns error 500', async () => {
    server.get('propertyies', () => new Response(500, {}, 'Server is down'));
    const { queryAllByTestId, queryByText } = render(Properties);
    await wait(() => queryAllByTestId('property'));
    expect(queryAllByTestId('property').length).toBe(0);
    expect(queryByText('Server is down').length).toBe(1);
  });
});
